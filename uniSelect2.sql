...
-- Use a join on all result columns
...
3.
-- MySQL does not support the MINUS or EXCEPT operators
...
(
      SELECT StdNo AS PerNo, StdFirstName AS FirstName, 
             StdLastName AS LastName, StdCity AS City, StdState AS State
        FROM Student );
...
-- Extra example 4
-- Row condition in the HAVING clause
SELECT CourseNo, Enrollment.OfferNo, 
       Avg(StdGPA) AS AvgGPA
